...
Let's model a realistic scenario: a household with 800W plug-in solar and a 5 kWh battery like the [EcoFlow DELTA 2](https://www.awin1.com/cread.php?awinmid=51797&awinaffid=2846734&ued=https%3A%2F%2Fwww.ecoflow.com%2Fuk%2Fdelta-2-portable-power-station).

**Daily energy flow on Flux:**

1. **5 AM–4 PM**: Solar generates ~3–4 kWh (depending on season and weather). You self-consume first (saving 15–18p/kWh), then export excess to the grid at day rate (~15–18p/kWh). Net saving: 15–18p per kWh exported.

2. **Night (10 PM–5 AM)**: Cheap electricity at 7–8p/kWh. If you have capacity, you charge your battery from the grid (if it's depleted) or don't need to.
...
3. **Smart home integration** (or willingness to manually manage charging/discharge). Some households manage this with simple timers, but dynamic optimisation requires Octopus's app or integration with a system like Home Assistant.

4. **Reliable solar generation**. Flux is designed for homes that generate meaningfully during daylight. If you're north-facing or heavily shaded, you won't generate enough to make the premium export payments worthwhile.
